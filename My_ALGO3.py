...
MAX_VOLUME = 5000
TOTAL_VOLUME = 25000
COUNT = int(TOTAL_VOLUME/MAX_VOLUME)
MAX_ORDERS = 5
SPREAD = 0.05
number_of_orders = 0
total_speedbumps = 0
...
def speedbumps(transaction_time):
    ...
    order_speedbump = -transaction_time + 1/MAX_ORDERS
    
    total_speedbumps = total_speedbumps + order_speedbump
    
    number_of_orders = number_of_orders + 1
    
    time.sleep(total_speedbumps/number_of_orders)
...
def buy_sell(session, sell_price, buy_price):
    for i in range(MAX_ORDERS):
        session.post('http://localhost:9999/v1/orders', params = {'ticker':'AC', 'type':'LIMIT', 'quantity': MAX_VOLUME,'price': sell_price, 'action': 'SELL'})
        session.post('http://localhost:9999/v1/orders', params = {'ticker':'AC', 'type':'LIMIT', 'quantity': MAX_VOLUME,'price': sell_price, 'action': 'BUY'})
...
def re_order(session, number_of_orders, ids, volumes_filled, volumes, price, action):
    for i in range(number_of_orders):
        ...
        if deleted.ok:
            session.post('http://localhost:9999/v1/orders', params = {'ticker':'AC', 'type':'LIMIT', 'quantity': volume,'price': price, 'action': action})
...
def market_making():
    ...
    with requests.Session() as s:
        ...
        while tick > 5 and tick < 295 and not shutdown:
            ...
            bid_price, ask_price = ticker_bid_ask(s,'AC')
            start = time.time()
