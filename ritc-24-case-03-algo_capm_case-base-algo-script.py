...
def main():
    with requests.Session() as session:
        ...
        try:
            ...
            capm_df = pd.DataFrame(columns=['Tick','ALPHA', 'GAMMA', 'THETA','Price-ALPHA', 'Price-GAMMA','Price-THETA', 'Price-RITM', 'Latest-News'])
        while get_tick(session) < 600 and not shutdown:
            ...
            ##update capm_df bid-ask dataframe
            # if expected_return['ALPHA']
            ...
            if current_tick != previous_tick:
                ...
                if capm_df['ALPHA'].empty or (capm_df_p['GAMMA'].iloc[0] != capm_df['GAMMA'].iloc[0]) :
                    ...
                    if capm_df.shape[0] >= 3001:
                        capm_df = capm_df.iloc[:3000]
                previous_tick = current_tick       
                    
                capm_df.to_csv(f'capm_df.csv', index=False)
